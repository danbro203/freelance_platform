...
<a class="btn btn-primary btn-lg" href="/register">Начать работу</a>
<a class="btn btn-outline-secondary btn-lg" href="/orders">Посмотреть заказы</a>
</div>
{% endblock %}''',
...
<small class="text-muted">Разрешённые форматы: PDF, DOC, DOCX, TXT, ZIP, PY, JS, HTML, CSS, JAVA, CPP, C, JSON, XML</small>
</div>
...
<ul class="pagination justify-content-center">
<li class="page-item {% if not pagination.has_prev %}disabled{% endif %}">
<a class="page-link" href="?page={{pagination.prev_num}}&search={{search}}&status={{status}}">Назад</a>
</li>
{% for p in pagination.iter_pages(left_edge=1, right_edge=1, left_current=1, right_current=2) %}
{% if p %}
<li class="page-item {% if p == pagination.page %}active{% endif %}">
...
{% endfor %}
<li class="page-item {% if not pagination.has_next %}disabled{% endif %}">
<a class="page-link" href="?page={{pagination.next_num}}&search={{search}}&status={{status}}">Вперёд</a>
</li>
</ul>
</nav>
{% endif %}
{% else %}
<div class="alert alert-warning">Заказов не найдено</div>
{% endif %}
{% endblock %}''',
...
{% if order.file_path %}
<p><strong>Файл заказчика:</strong> 
<a href="/download/{{order.id}}" class="btn btn-sm btn-outline-success">📎 Скачать</a>
</p>
{% endif %}
</div>
</div>
...
{% if order.file_path %}
<p><strong>Прикреплённый файл:</strong> 
<a href="/download/{{order.id}}" class="btn btn-sm btn-outline-primary">📎 Скачать файл</a>
</p>
{% endif %}
<hr>
{% if order.status == 'in_progress' %}
<a href="/complete_order/{{order.id}}" class="btn btn-success" onclick="return confirm('Завершить заказ?')">Завершить заказ</a>
{% endif %}
{% if order.status == 'completed' %}
{% set has_review = order.reviews|length > 0 %}
{% if not has_review %}
<a href="/add_review/{{order.id}}" class="btn btn-warning">Оставить отзыв</a>
{% else %}
<span class="badge bg-success">Отзыв оставлен</span>
{% endif %}
{% endif %}
</div>
</div>
...
<div class="card-body">
<h5 class="card-title">
<a href="/worker/{{r.worker.id}}">{{r.worker.username}}</a>
{% if r.worker.average_rating() > 0 %}
<small class="text-muted">⭐ {{r.worker.average_rating()|round(1)}}</small>
{% endif %}
</h5>
...
<div class="card-body">
<p><strong>Заказ:</strong> {{order.title}}</p>
<p><strong>Исполнитель:</strong> {{worker.username}}</p>
<hr>
<form method="POST">
{{form.hidden_tag()}}
<div class="mb-3">
<label class="form-label">{{form.rating.label.text}}</label>
...
{{form.comment(class="form-control", rows=4, placeholder="Опишите ваш опыт работы...")}}
</div>
...
{% endblock %}''',
    ...
    'worker_profile.html': '''{% extends "base.html" %}
{% block content %}
<div class="row">
<div class="col-md-4">
<div class="card shadow">
...
<p class="text-muted">Исполнитель</p>
<hr>
<h4>
{% if avg_rating > 0 %}
{{ "⭐" * avg_rating|int }}
...
<p class="small text-muted">На основе {{reviews|length}} отзывов</p>
</div>
</div>
</div>
<div class="col-md-8">
